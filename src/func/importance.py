...
def get_importance(gate_weight: torch.Tensor, up_weight: torch.Tensor, down_weight_t: torch.Tensor, weights: Optional[list] = [1.0, 1.0, 1.0]) -> torch.Tensor:
    """
    ...
    gate_max_abs = torch.sqrt(torch.max(gate_weight, dim=1).values.pow(2) + 
        torch.min(gate_weight, dim=1).values.pow(2)) * weights[0]
    
    up_max_abs = torch.sqrt(torch.max(up_weight, dim=1).values.pow(2) + 
        torch.min(up_weight, dim=1).values.pow(2)) * weights[1]

    down_weight_t = torch.sqrt(torch.max(down_weight_t, dim=1).values.pow(2) + torch.min(down_weight_t, dim=1).values.pow(2)
    ) * weights[1]

    return torch.sqrt(gate_max_abs.pow(2).float() + up_max_abs.pow(2).float() + down_weight_t.pow(2).float())
...
def get_adjusted_importance(
    gate_weight: torch.Tensor, up_weight: torch.Tensor, down_weight_t: torch.Tensor, weights: list = [1.0, 1.0, 1.0]
) -> torch.Tensor:
    """
    ...
    return torch.sqrt(gate_importance.pow(2).float() + up_importance.pow(2).float() + down_importance.pow(2).float())
...
def get_adjusted_importance_2_with_gradients(
    ...
    gate_importance = get_adjusted_weight_importance_2_with_gradients(gate_weight, activations=activations[0], gradients=gradients[0]) * weights[0]
    up_importance = get_adjusted_weight_importance_2_with_gradients(up_weight, activations=activations[0], gradients=gradients[0]) * weights[1]
    down_importance = get_adjusted_weight_importance_2_with_gradients(down_weight_t.t(), activations=activations[0], gradients=gradients[0]) * weights[2]
...
def get_adjusted_weight_importance(weight: torch.Tensor) -> torch.Tensor:
    ...
    adjusted_importance = torch.sqrt(
        (positive_max.pow(2).float() + negative_max_abs.pow(2).float() + weight_adjusted_mean.pow(2).float())
        * weight.size(1)
        / weight_small_count
    )
...
def get_adjusted_weight_importance_2(weight: torch.Tensor) -> torch.Tensor:
    ...
    # Normalize weights
    weight_normed = weight / (torch.norm(weight, p=2, dim=1, keepdim=True) + 1e-8)  # Shape [N, M]
...
def get_adjusted_weight_importance_2_with_gradients(weight, activations=None, gradients=None, eps=1e-8):
    ...
    activation_importance, grad_importance = process_activations_and_gradients(activations, gradients, device=weight.device)
    ...
    # Normalize importance scores
    activation_importance = activation_importance / (torch.max(activation_importance) + eps)
...
def process_activations_and_gradients(
    ...
    act_tensor = torch.cat([act.reshape(-1, act.size(-1)).to(device) for act in activations])
    grad_tensor = torch.cat([grad.reshape(-1, grad.size(-1)).to(device) for grad in gradients])
    
    # Calculate statistics for each neuron
    act_stats = torch.stack([
        act_tensor.mean(dim=0),
        act_tensor.std(dim=0),
        act_tensor.abs().mean(dim=0)
    ])  # Shape: [3, hidden_dim]
    
    grad_stats = torch.stack([
        grad_tensor.mean(dim=0),
        grad_tensor.std(dim=0),
        grad_tensor.abs().mean(dim=0)
    ])  # Shape: [3, hidden_dim]
