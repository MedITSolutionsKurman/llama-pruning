...
def map_iio_steps(examples: List[dict]) -> str:
    ...
    for idx, example in enumerate(examples):
        print(example)
        if idx == 0:
           output.append(example['step'].replace('<step>', '<thinking>').replace('<p>', '').replace('</p>', '').replace('</step>', '</thinking>'))
           continue

        if len(examples) > 2 and idx > 0 and idx < len(examples) - 1:
            output.append(example['step'].replace('<step>', '<reflection>').replace('<p>', '').replace('</p>', '').replace('</step>', '</reflection>'))
        else:
            output.append(example['step'].replace('<step>', '<output>').replace('<p>', '').replace('</p>', '').replace('</step>', '</output>'))
